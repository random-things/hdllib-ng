...
the enum: `OpUnloadDll = 92` and `OpFingerprint = 93`. Never infer opcode values
from source order.
...
|---|---|---|---|---|
| Lifecycle, quiet defaults, logging | `HdlInit`, `HdlShutdown`, `HdlStartIpc`, log and health controls in [`hdllib.h`](../include/hdllib/hdllib.h) | [`dllmain.cpp`](../src/dllmain.cpp), [`core.cpp`](../src/core.cpp), [`env.cpp`](../src/env.cpp), [`log.cpp`](../src/log.cpp) | Basic handlers in [`handlers_basic.cpp`](../src/ipc/handlers_basic.cpp); `ping`/`log` in [`cmds_basic.cpp`](../tools/client/cmds_basic.cpp) | [architecture](architecture.md), [`tests/test_main.cpp`](../tests/test_main.cpp) |
| Injection, target resolution, recommendation, unload/reload | `HdlInjectDll*`, `HdlResolveTarget`, `HdlRecommendInject`, `HdlUnloadDll` | [`inject.cpp`](../src/inject.cpp), [`inject/select.cpp`](../src/inject/select.cpp), one implementation per method in [`src/inject/`](../src/inject/) | Local controller path in [`local_inject.cpp`](../tools/client/local_inject.cpp); IPC inject/unload in [`handlers_basic.cpp`](../src/ipc/handlers_basic.cpp) | [injection index](inject/README.md), [selection](inject/selection.md), [`test_select.cpp`](../tests/test_select.cpp), injection matrix in [`test_main.cpp`](../tests/test_main.cpp) |
