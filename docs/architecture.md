...
should call the `Control/Shutdown` RPC / `HdlShutdownEx` **before** `FreeLibrary` so teardown
...
flowchart TD
    ...
    Prepare["Control/Shutdown or HdlShutdownEx"] --> WaitBoot["Wait for bootstrap done"]
...
then closes session and local-job maps.
...
sequenceDiagram
    ...
    Cmd->>Cmd: Populate declared request message
    Cmd->>PC: Generated unary or server-streaming call
    PC->>Frame: uint32 size + protobuf Envelope
    Frame->>Dispatch: parsed named Request
    Dispatch->>Dispatch: parse generated method name
    Dispatch->>Handler: generated method dispatch
    Handler->>Handler: validate and decode fields
    Handler->>Domain: call C++ domain function
    Domain-->>Handler: HdlStatus and result data
    Handler-->>PC: one Response or streamed Responses
...
| RPC schema and method inventory | [`proto/hdl/rpc/v1`](../proto/hdl/rpc/v1), generated by [`tools/rpc_codegen`](../tools/rpc_codegen) |
| Envelope runtime and generated dispatch | [`src/rpc`](../src/rpc), [`src/ipc/dispatch.cpp`](../src/ipc/dispatch.cpp) |
| Request validation and response encoding | [`src/ipc/handlers_*.cpp`](../src/ipc/) |
| Shared client transport and stream reassembly | [`src/rpc/pipe_client.cpp`](../src/rpc/pipe_client.cpp) |
| Human command parsing and presentation | [`tools/client/cmds_*.cpp`](../tools/client/) |

### Wire invariants

- A connection begins with `HDLRPC1\n`, then a length-prefixed protobuf
  `ClientHello`; the server answers with `ServerHello` before accepting calls.
- Every later frame is `uint32_t size` followed by a serialized protobuf
  `Envelope`. Requests carry a nonzero request ID and a generated fully qualified
  method name.
- Responses repeat the request ID and carry `RpcStatus`; `end_stream` marks the
  final response.
- Request and response payloads directly contain the serialized protobuf type
  declared by the selected method. UTF-8/UTF-16 conversion happens only at the
  typed IPC handler boundary.
- `Request.timeout_ms == 0` means no deadline. Search methods intentionally have
  no default timeout.
- Streaming eligibility comes from `services.proto`; ordered `Response`
  envelopes continue until `end_stream` is true.
- Unknown method names return `UNIMPLEMENTED`; malformed requests return
  `INVALID_ARGUMENT` and retain the mapped `HdlStatus` where applicable.

- Generated handler symbols are service-qualified (`Handle<Service>_<Method>`),
  so two services may use the same method name without a C++ symbol collision.

`PipeClient` lives in the shared RPC runtime. The CLI and the controller-local
unload helper both use that channel and generated service clients; unload does
not maintain a separate handshake or envelope implementation.
...
| Tracked loaded modules | [`loaded_modules.cpp`](../src/loaded_modules.cpp) | Mutex | `UnloadTrackedExcept` / `Control/Shutdown` |
